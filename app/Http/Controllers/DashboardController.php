...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($users as $user) {
            ...
            $db_date        = $user->datum;
            $trial          = $user->trial;
            $active_user    = ($trial == 0 && strtotime($today) < strtotime($db_date)); // Boolean

            switch(true) {
                case $trial == 1 && strtotime($today) < strtotime($db_date):
                    $user_trial++;
                    break;
                case $trial == 0 && strtotime($today) < strtotime($db_date):
                    $user_active++;
                    break;
                case strtotime($db_date) < strtotime($today) && strtotime($db_date) > strtotime($expired_date):
                    $user_just_expired++;
                    break;
                default:
                    $user_expired++;
